...
hyper4k (Kotlin/Native 项目)   可复用的封装（Hyper4kServer / Request / Response）
   ↓ 依赖
neton-http-hyper4k             Neton Adapter，接进 RouteMatcher / RequestEngine
```
...
/* 流式响应：begin 之后逐块 write，最后 finish。 */
...
迟到调用安全失败而不会碰到已释放的内存。crate 以 `panic = "abort"` 编译，保证 panic 不跨 FFI。
...
`neton-http-hyper4k` 是 Neton 仓库中的一个模块，也是 Neton 的**默认**服务端引擎：应用只要
依赖该模块，无参的 `http { }` DSL 就会解析到 `Hyper4kHttpAdapter`。显式写出 Adapter 等价：
...
配置或运行时 Provider 注册；未引入该 Adapter 的应用不会链接 Rust/FFI 产物。
