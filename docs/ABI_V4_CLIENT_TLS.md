...
- 建立三层基线：纯 Hyper、hyper4k C ABI、Neton 完整链路，分别记吞吐、p99、RSS、
  ...
  **已建立**（`lib/benches/three_layer.rs`，macOS arm64，20k 请求 / 32 连接 /
  keep-alive h1，5 轮）：

  | 层 | 吞吐 | p50 | p99 | RSS |
  |---|---|---|---|---|
  | 1 纯 Hyper | 115k–129k rps | 239–263 µs | 423–487 µs | ~10.0 MiB |
  | 2 hyper4k C ABI | 116k–134k rps | 227–257 µs | 451–567 µs | ~12.2 MiB |
  | 3 Neton 完整链路 | 由 Neton 仓驱动，不在本仓 | | | |

  相对纯 Hyper 的吞吐差 5 轮分别是 −14.2% / −9.0% / −5.2% / −3.4% / +10.8%
  （负号表示 hyper4k 更快）。**结论：在这个负载下 C ABI 的开销落在运行间噪音里，
  测不出来。** RSS 稳定高约 2.2 MiB，那是 Tokio runtime 与 DashMap 的固定成本。

  > 第一次只跑了一轮，得到 p99 1178 µs、吞吐 −17.3%，我据此在文档里写下了
  > "p99 涨 2.6 倍" 的结论。第二轮就反了过来（layer 2 反而快 8.1%）。
  > **那个尖峰是离群值，不是发现。** 单次测量不足以支撑任何性能结论，本表因此
  > 记的是区间而不是单点。
  >
  > 这不代表 §五 列的优化没有价值 —— header 文本编码等每请求成本仍然真实存在，
  > 只是**这个负载测不出它们**。要看到它们需要 header 更多、body 更大或并发更高的
  > 场景；在那之前不应假设它们是瓶颈。
