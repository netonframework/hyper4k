...
> 状态：**§三（Rust）、§3.3（h2c）、§4.1/4.2（Kotlin live response）、§4.4 能力声明
> 均已完成**，一致性套件的流式用例在 hyper4k 上真跑（不是 skipped），能力是挣来的。
> 剩 §五 切引擎，以及 Ktor 侧接同一份套件。
...
第 1 条：套件已补上具体断言与流式钩子。`roundTrip` 表达不了流式——它只有
「请求进、响应出」，缓冲和真流式的返回值一模一样，差别只在时间里。所以加了
`streamRoundTrip` 和 `ChunkMeter.released()`：报告传输层已交给下游的块数，
缓冲实现在产出期间恒为 0。hyper4k 的实现起真实服务器、连真实 socket、
解析 chunked 分帧来数块，不是包一层假通道。

这条断言验过有牙齿：把 `Hyper4kLiveResponse.stream` 临时改成「攒完再写」，
`streamingReleasesChunksAsProduced` 立刻失败，其余五条照常通过。

尚未完成：Ktor CIO 侧还没接同一份套件，所以「两个引擎跑同一份断言」目前只有一半。
第 5 条（并发 100 条 SSE 下普通请求 p99 不劣化）还没有对应用例。
